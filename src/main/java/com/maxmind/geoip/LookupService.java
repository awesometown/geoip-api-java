...
public class LookupService {
	 ...
	 * Database file.
	 */
	private RandomAccessData db = null;
	...
	public LookupService(RandomAccessData database) throws IOException {
		this.db = database;
		dnsService = 0;
		init();
	}
	...
	// this.db = new RandomAccessFile(databaseFile, "r");
	// init();
	// }
	...
	public Country getCountryV6(InetAddress addr) {
		if (db == null && (dboptions & GEOIP_MEMORY_CACHE) == 0) {
...
//					/* refresh filehandle */
//					close();
//					db = new RandomAccessFile(databaseFile, "r");
//					databaseInfo = null;
//					init();
//				}
//			}
	...
	public synchronized Location getLocationV6(InetAddress addr) {
		...
		try {
			...
			if ((dboptions & GEOIP_MEMORY_CACHE) == 1) {
				...
				// read from disk
				db.seek(record_pointer);
				db.readFully(record_buf);
			}
